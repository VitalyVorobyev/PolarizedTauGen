#define selfit_cxx
// The class definition in selfit.h has been generated automatically
...
//
// Root > T->Process("selfit.C")
// Root > T->Process("selfit.C","some options")
// Root > T->Process("selfit.C+")
//

#include "selfit.h"

void selfit::Begin(TTree * /*tree*/)
{
...
void selfit::SlaveBegin(TTree * /*tree*/)
{
...
Bool_t selfit::Process(Long64_t entry)
{
   // The Process() function is called for each entry in the tree (or possibly
   // keyed object in the case of PROOF) to be processed. The entry argument
   // specifies which entry in the currently loaded tree is to be processed.
   // It can be passed to either selfit::GetEntry() or TBranch::GetEntry()
   ...
   double D0 = (d0*A_ + omega*xirho_sm*(F.Vect()*B_))*A0*BPS;      // 1 

   double D1 = (d0*A_ + omega*xirho_sm*(F.Vect()*B_))*A1*BPS;      // rho 

   double D2 = (d0*A_ + omega*xirho_sm*(F.Vect()*B_))*A2*BPS;      // eta 

   double D3 = (xirho_sm*dnlb0 + omega*A_*(G.Vect()*Nlep))*B1*BPS; // ksi 

   double D4 = (xirho_sm*dnlb0 + omega*A_*(G.Vect()*Nlep))*B2*BPS; // ksi*del 
